...
public class PartieView implements Screen
{
	...
	public PartieView(TAGame game, Partie partie)
	{
		...
		camera.up.set(0, 1, 0);
		updateCamera();
		fenetreToolShop = new ToolShopWindow(partie.getToolShop(), partie, Gdx.graphics.getHeight() - 100, Gdx.graphics.getWidth() - 100);
	...
	public void render(float delta)
	{
		...
		if (afficheGrille)
		{
			for (Box b : partie.getTerrain().getBox())
			{
				...
				{
					shape.line(new Vector3(b.getTabCoordX()[i], 0f, b.getTabCoordY()[i]),
					            new Vector3(b.getTabCoordX()[i + 1], 0f, b.getTabCoordY()[i + 1]));
				}
			}
			...
			for (Box b : partie.getTerrain().getBox())
			{
				...
				font.draw(batch, String.valueOf(b.getRange()), b.getCoordX() - partie.getTerrain().getHalfRadiusPolygon(), -b.getCoordY() + 2);
			}
			batch.end();
		}
		shape.end();
		...
		shape.circle(partie.getTerrain().getFinishBox().getCoordX(), partie.getTerrain().getFinishBox().getCoordY(), partie.getTerrain()
			...
			for (Monster m : partie.getTerrain().getMonsters())
			{
				...
				synchronized (m)
				{
					for (Box b : m.getPath())
					{
						...
						font.draw(batch, String.valueOf(compteur), b.getCoordX() - partie.getTerrain().getHalfRadiusPolygon() / 2,
						            -b.getCoordY());
						compteur++;
					}
				}
