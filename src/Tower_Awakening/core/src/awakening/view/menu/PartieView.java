...
public class PartieView implements Screen {
	...
	public PartieView(TAGame game, Partie partie) {
		...
		espace = MeshTA.loadMeshTA(Gdx.files.internal("espace.mta")
				.file());
		...
		camera.up.set(0, 1, 0);
		updateCamera();
		fenetreToolShop = new ToolShopWindow(partie.getToolShop(), partie,
				Gdx.graphics.getHeight() - 100, Gdx.graphics.getWidth() - 100);
	...
	public void render(float delta) {
		synchronized (partie) {
			if (boxIsClicked) {
				if(onHoveredBox!= null)
					partie.clickBox(onHoveredBox);
					boxIsClicked = false;
			}
		}
		...
		for (Tower t : partie.getJoueur().getTowerIterator()) {
			...
			synchronized (t.getProjectiles()) {
				for (Projectile p : t.getProjectiles())
					p.render(renderer, camera.combined);
			}
		}
		...
		if (afficheGrille) {
			for (Box b : partie.getTerrain().getBox()) {
				for (int i = 0; i < b.getTabCoordX().length - 1; i++) {
					shape.line(
							new Vector3(b.getTabCoordX()[i], 0f, b
									.getTabCoordY()[i]),
							new Vector3(b.getTabCoordX()[i + 1], 0f, b
									.getTabCoordY()[i + 1]));
				}
			}
			...
			for (Box b : partie.getTerrain().getBox()) {
				...
				font.draw(batch, String.valueOf(b.getRange()), b.getCoordX()
		...
		if(onHoveredBox != null)
		{
			shape.setColor(0,0.5f,0.5f,0.5f);
			for (int i = 0; i < onHoveredBox.getTabCoordX().length - 1; i++) {
				shape.point(onHoveredBox.getTabCoordX()[i], 0.2f , onHoveredBox
								.getTabCoordY()[i]);
				shape.point(onHoveredBox.getTabCoordX()[i + 1], 0.2f, onHoveredBox
								.getTabCoordY()[i + 1]);
			}
		}
		shape.end();
		...
		shape.circle(partie.getTerrain().getFinishBox().getCoordX(), partie
				.getTerrain().getFinishBox().getCoordY(), partie.getTerrain()
		...
		synchronized (partie.getTerrain()) {
			for (Monster m : partie.getTerrain().getMonsters()) {
				...
				synchronized (m) {
					for (Box b : m.getPath()) {
						shape.circle(b.getCoordX(), b.getCoordY(), partie
								.getTerrain().getHalfRadiusPolygon());
						font.draw(batch, String.valueOf(compteur),
								b.getCoordX()
										- partie.getTerrain()
												.getHalfRadiusPolygon() / 2,
								-b.getCoordY());
						compteur++;
					}
				}
	...
	public void chooseBox(int x, int y)
	{
		mousePosition = this.getUnprojectedPoint(x, y);
		int i = partie.getTerrain().getBoxIndexByPosition(mousePosition.x, mousePosition.z);
		if(i>=0 && i < partie.getTerrain().getBox().size())
			onHoveredBox = partie.getTerrain().getBox().get(i);
	}
}
