...
public class WaitingRoomServer implements Screen
{
	...
	public void show() 
	{
		...
		btnSend.setPosition(btnBack.getX(), btnBack.getY()+ btnBack.getHeight() + 5);
		btnSend.addListener
		(
				new ClickListener() 
				{
					...
					public boolean touchDown(InputEvent e, float x, float y, int pointer, int button)
					{
						...
						{
							TextArea t = new TextArea("Server : " + message.getText(),skin); 
		...
		btnStart.setPosition(btnSend.getX()+btnSend.getWidth(), btnSend.getY());
